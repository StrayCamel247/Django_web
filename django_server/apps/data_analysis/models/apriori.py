...
# __description__ : Apriori 算法是一种最有影响力的挖掘布尔关联规则的频繁项集的 算法,它使用一种称作逐层搜索的迭代方法，k- 项集用于探索（k+1）- 项集。首先，找出频繁 1- 项集的集合。该集合记作L1。L1 用于找频繁2- 项集的集合 L2，而L2 用于找L2，如此下去，直到不能找到 k- 项集。
...
def apriori(dataSet, minSupport=0.5):
    ...
    while (len(L[k-2]) > 0): # 一直迭代到项集数目过大而在事务集中不存在这种n项集
        ...
        k += 1
    res = {'origin_data':dataSet, 'supportData':supportData, 'description':"", 'res_data':L,'minSupport':minSupport}
    return res
...
if __name__=='__main__':
    ...
    # print(L1,suppDat)
    print(apriori(dataSet,minSupport=0.7))